...
# TTS components removed - using simple frontend-based approach
...
# TTS system simplified - no complex initialization needed
...
class SpeakRequest(BaseModel):
    ...
    use_segmentation: Optional[bool] = True
    voice: Optional[str] = "default"

class StreamingSpeakRequest(BaseModel):
    text: str
...
async def speak(request: SpeakRequest):
    ...
    if engine == "edgetts":
        ...
        await broadcast_to_vrm({
            "type": "speak",
            "data": {
                "chunkId": f"chunk_{int(__import__('time').time() * 1000)}",
                "text": text,
...
# ============= Frontend Segmentation Support =============
# /api/speak now serves as the only TTS endpoint for frontend-segmented text
